...
- **차트 및 데이터 시각화** — `recharts`, `chart.js` + `react-chartjs-2`, `d3`
- **지도** — `leaflet` + `react-leaflet`
- **비디오** — `react-player`
- **애니메이션** — `framer-motion`, `react-spring`, `lottie-react`, `react-confetti`, `tailwindcss-animate`
- **리치 텍스트** — `@tiptap/react` + `@tiptap/starter-kit`
- **폼** — `react-hook-form` + `@hookform/resolvers` + `zod`
- **플로 다이어그램** — `@xyflow/react`
- **3D** — `three`, `@react-three/fiber`, `@react-three/drei`
- **이미지** — `react-easy-crop`, `react-image-gallery`, `yet-another-react-lightbox`
- **PDF** — `jspdf`
- **캘린더** — `react-big-calendar`, `react-day-picker`
- **CSS-in-JS** — `styled-components`, `@emotion/react`
- **유틸리티** — `date-fns`, `lodash`, `axios`, `zustand`, `react-router-dom`, `@faker-js/faker` 등 다수
