...
- **チャートとデータ可視化** — `recharts`、`chart.js` と `react-chartjs-2`、`d3`
- **地図** — `leaflet` と `react-leaflet`
- **動画** — `react-player`
- **アニメーション** — `framer-motion`、`react-spring`、`lottie-react`、`react-confetti`、`tailwindcss-animate`
- **リッチテキスト** — `@tiptap/react` と `@tiptap/starter-kit`
- **フォーム** — `react-hook-form` と `@hookform/resolvers`、`zod`
- **フロー図** — `@xyflow/react`
- **3D** — `three`、`@react-three/fiber`、`@react-three/drei`
- **画像** — `react-easy-crop`、`react-image-gallery`、`yet-another-react-lightbox`
- **PDF** — `jspdf`
- **カレンダー** — `react-big-calendar`、`react-day-picker`
- **CSS-in-JS** — `styled-components`、`@emotion/react`
- **ユーティリティ** — `date-fns`、`lodash`、`axios`、`zustand`、`react-router-dom`、`@faker-js/faker` など多数
